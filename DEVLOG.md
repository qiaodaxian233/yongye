...
## m392 美术占位替换(3A 打磨路线图第 10 项,2026-07-31)
- **精英皮肤程序化染色(SKILL.md 红线=UV 皮肤图集不能 AI 平面图,走程序化管线)**:m160 遗留的四张纯色占位(精英怪叠皮后整只一坨纯色)全部换真皮肤——原版 creeper/skeleton/spider/witch 皮肤从 misode/mcmeta 1.21.1-assets 拉取(m307 noise_settings 同源先例),lum(Rec.709)重映射夜蚀精英紫四段色阶(10,6,22→74,42,128→150,96,216→214,182,255,与精英血条 0xB44CFF/紫气同族)+轻对比拉伸防灰蒙(m210 同思路)+30% 原色保线稿(m280 同手法);**alpha 逐像素零改动已校验四张全 True**(m210 铁律:轮廓零改动);顺手修尺寸错误=elite_witch 占位是 64×64 而原版女巫 UV 声明 64×128,现按真尺寸落图。elite_zombie(m280 已是成品)不动。
- **accessory_gui 按真实槽位重绘**:病根=旧代码 drawBackground 对每个槽位画平灰 fill(0xFF8B8B8B 边+0xFF373737 芯)把贴图槽位区全盖死——贴图画什么都白画,「占位没做」的观感由此而来。修两头:①176×158 背景图程序化重绘(m142 玻璃语汇=渐变玻璃蓝面板+2px 切角+顶高光,神器 2×5 区/鞘翅暗红区/背包热栏区三块衬板,**31 个槽位凹槽逐一按 AccessoryScreenHandler 真实坐标烘进图**=神器 44+18c/18与38、鞘翅 152,28、背包 8+18c/70+18r、热栏 132,凹槽=描边+内阴影+底缘微光);②AccessoryScreen 撤平灰覆盖循环,只保留鞘翅空槽「翼」提示(色提亮 0xFFB35A5A 在暗红凹槽上可读)。标题行 y6/背包标题行 y60 留空给文字。
...
## m393 HUD 微动效(3A 打磨路线图第 12 项,第一批收官,2026-07-31)
- **三处既有 HUD 只加动效不改布局**(路线图防重复列口径),统一走 enableHudMicroFx && FxBudget.on() 双门,关=逐处回旧观感:
- **①看板击杀数滚动+增量亮闪**:旧=totalKills 硬跳;新=显示值向真值指数趋近滚上去(dt*9 指数+每秒 4 杀保底速度防长尾,差<0.5 收口钳制),增量瞬间击杀字色 0xFFFF7070→0xFFFFE9A0 暖金亮闪 0.4s 回落(mixColor 纯算术小工具);首帧/重连回退直贴不滚,nanoTime 驱动帧率无关。
- **②CD 转好整面白闪**:m353 金框闪(①层描边)保留,叠加=readyFlash 12t 期间芯片整面白纱 200→14 线性衰减(画在④高光后文字前,字压闪上保读),「转好了」从框亮升级成整片点亮。
- **③连击弹簧弹跳**:旧=comboPopTicks 4t 线性(tick 量化 20fps 生硬);新=nanoTime 300ms 衰减余弦 1.15+0.55·e^(-3.2t)·cos(2πt)=1.70 猛胀→过 1.15 微缩 squash→回位,钳 ≥1.0,亚 tick 平滑;comboPopTicks 保留并行走=关微动效无缝回 m273 线性。与 enableComboFancyFx(环/称号/抖动)正交。
- 配置+1(enableHudMicroFx 默认开),**configVersion 136→137**;设置屏「界面·HUD」UI 动效区加 2 钮。零新 API 面(fill/drawText/矩阵缩放/纯算术全在树);括号自检 3 文件全平。
- 实机盯:刷一波怪看击杀数滚上去且字短暂发金、单杀+1 看 0.1s 内滚完不拖、重连看数字直贴不倒滚、技能转好看芯片整面白闪且金框仍在、连击连打看数字弹簧感(胀-缩-回)对比关微动效的线性、质量档 0 三处全回旧、看板紧凑/完整两模式都对。

## m393 附:CI 绿入册,第一批收官(2026-07-31)
- m393(3724067)云端构建 success(零新 API 面,CI=编译回归);路线图第 12 项 🛠→🧪。**至此 3A 打磨第一批(3/6/22/7/9/11/13/15/16/18/8/10/12)全部完成入 🧪**,待实机验收后逐项转 ✅;下一批=第二批 14(受击顿帧扩展)/17/19/20/21。

## m394 疾跑收刀背挂回修(作者实机截图:「肉盾武器怎么飞起来了,不在手上拿着」,2026-07-31)
- **现象定性**:截图=m247/m327「疾跑收刀·背后」样式(sprintWeaponStyle=1,疾跑时主手武器收背后斜挎、手上藏掉,WeaponBackFeatureRenderer+WeaponSheathMixin 共用 shouldSheath)。不是武器丢了,是背挂观感烂+条件残留,作者认不出这是功能。
- **病根①净角算错**:旧 Z 旋转 -125° 想当然按「物品竖直起算」,但 FIXED 展示模式下剑类模型**本身自带 45° 斜置**(GUI 同款对角朝向),净角≈水平——普通剑还凑合,肉盾巨阙这类大模型职业武器直接「横平悬空一大把」。修=默认角 -80°(净效果=斜挎柄朝右肩),且**角度/缩放/下移/贴背四值全配置化**(weaponBackAngleDeg/-Scale/-DownOff/-BackOff,原类注释自己写着「不合适截图回来调这三个常量」——截图回来了,顺势把常量升配置,设置屏「背挂微调」新分区 12 钮即点即看,含「角·旧-125」一键回旧对比);缩放默认 0.85→0.75(大模型收敛),贴背 0.19→0.22。
...
## m396 第一人称姿态泄漏回修 + m394 诊断勘误(作者两截图,2026-07-31)
- **现象**:第一人称整屏横贯一把"大刀"(图1 紫=龙魂染色档、图2 红=高强化染色档),调设置无效;m394 那张第三人称"武器悬空"同源。
- **排查路径(入册防再走弯路)**:先后排除背挂渲染器(仅第三人称)、龙魂/混沌刃模型(实为 handheld 平面贴图不可能巨大)、刀光弧(SlashFxManager 是弧带贴图非刚体模型)、蓄力斩(只放大弧光)——真凶=**m260 站姿层**:手持刀光武器即循环播 yongye_battle_idle(按右键=guard_pose),playerAnimator 骨骼动画**从未做第一人称隔离**,循环姿态把武器横持在脸前;巨阙模型本身 35 单位长(合法,display 齐全,handheld 约 1.4 倍观感),被站姿横举后在第一人称=「整屏大刀」。**m394 勘误:那张截图不是背挂也不是拖刀残留,就是站姿横持巨阙**(m394 的移动门槛与背挂参数化仍是有效改进,不回滚)。
- **修①第一人称门·站姿**:tickStance 的 want 计算前加 `mc.options.getPerspective().isFirstPerson()` 门(yarn 已核 method_31044/method_31034)——第一人称恒无架势(切视角 4t 融出/融入),架势本是第三人称观感,自己第一人称只吃遮挡不吃收益;开关语义不变。
- **修②第一人称门·挥砍真动作**:playFor 同门返回 false 走程序化姿态(程序化只改第三人称模型角,第一人称保持原版挥手)。
- **修③开关补 UI**:slashFxBattleStance/slashFxGuardPose/slashFxAnimLib 三配置一直存在但设置屏没按钮=作者"调成默认也不行"的另一半原因——「武器架势」新分区 6 钮补齐。零配置字段变更,**v 仍 138**。
- **答疑入册·怪物伤害为何吃不满 3 亿护甲**:①原版护甲减伤硬上限 80%(护甲点进公式封顶,3 亿=30 点效果相同);②本模组「动态对位」(enableDynamicMobScaling 默认开)按设计=怪伤=玩家最大生命÷dynamicMobSurviveHits(默认 30)——1.1B 血→每下约 3700 万(护甲后约 1/5),**怪跟着你变强是"无限强化反向变强"的核心玩法不是 bug**;嫌疼调 dynamicMobSurviveHits 调大(50=被打 50 下才致命)或关 enableDynamicMobScaling。
- 实机盯:第一人称持巨阙看视野干净无横刀、F5 看战斗站姿/格挡姿态照常、切视角看 4t 平滑过渡、挥砍第一人称原版挥手第三人称七式照旧、设置屏「武器架势」6 钮即点即改、关战斗站姿看 F5 下持械垂手。

## m397 站姿默认改关(作者点名:「只保留七式,拿武器走路太难看,恢复默认」,2026-07-31)
- **默认改版**:slashFxBattleStance / slashFxGuardPose 默认 true→false——手持武器的备战架势循环与右键格挡横举姿态默认不再播放,**持械走路/站立回原版观感**;挥砍七式(真动作库 slashFxAnimLib + 程序化姿态)、疾跑姿态(m316)、跑时武器样式(m327 拖刀)均**不动**。功能保留没删:想要架势去设置屏「武器架势」分区(m396 补的 6 钮)一键开回。
- **迁移口径(重要,入册防再踩)**:布尔默认翻转**不能**用惯常的「值判等仅旧默认迁移」——true 区分不了"沿用旧默认"与"用户主动开",且不加版本门会每次启动都把用户开回的值又关掉。改为**版本门迁移**:configVersion<139 时统一关一次(按作者决定),之后用户再开即长期生效。**configVersion 138→139**。
- 零代码路径变更(纯配置默认+迁移),站姿系统/第一人称门(m396)原样;m396 的第一人称隔离在默认关后依然有意义=用户开回架势时第一人称照样干净。
- 实机盯:更新后进档持巨阙第三人称走路=原版摆臂持械、站立=原版垂手、按右键格挡=无横举动画(格挡功能本身照常)、挥砍七式照旧、疾跑拖刀照旧;设置屏点「战斗站姿·开」后 F5 架势回来且重启不被关回;第一人称始终干净。

## m398 巨阙/剑士刃 display 变换修复=「整屏大刀」终勘(作者四截图+命令排除法,2026-07-31)
- **定案证据链**:①四张截图里刀锁定在屏幕同一位置(低头看地它也不动)=镜头挂载;②颜色随强化染色档变(紫→红);③作者实测关站姿两命令无效——三条合并只剩一个渲染体:**第一人称手持物本身**。投影预览器把旧 display 值一喂,**逐像素复现截图**(整屏水平横杆),诊断由模拟复现坐实。
- **病根**:class_weapon_warrior(35 单位长 Z 轴向刃)/class_weapon_swordsman(21 单位)的 display 手调值全错——FP rotation[85,-5,80] 把刃转成水平并抬到视线高:1.3 格长横杆距脸 0.72 格,FOV70 下角宽 ~84°>整个视野=「整屏大刀」;TP [90,0,90] 同病横在胸口。**终勘两次误诊:m394 那张"悬空"不是背挂、m396 猜的站姿泄漏也不是主因,全是手持巨阙的错误 display**(m394 背挂参数化/移动门槛、m396 第一人称隔离、m397 站姿默认关三项改动本身各有价值,不回滚)。
- **修法(不再盲猜角度)**:写投影预览器复刻 MC display 管线(v/16−0.5 → S → R(rotationXYZ,向量先 Z 后 Y 后 X)→ T;FP 手锚 (0.56,−0.52,−0.72)、FOV70 透视),迭代出:FP rotation[65,25,0] scale 0.45/0.6(巨阙/剑士),TP [75,0,15] scale 0.55/0.75,左手镜像 Y/Z 反号;**平移不手调**——「握柄点归位」公式 t = target − R(S(grip)),巨阙握柄 (8,0,10.5)(柄环簇 z≈8~13)、剑士 (8,0,8.5)。GUI/ground/head/fixed 原值保留(图标一直正常)。修正前后预览三张入 docs/hud(m398_fp_bug_repro/fp_fixed/tp_fixed)已复核。
- **给作者的关键说明**:模型在 jar 里——游戏内命令改不了模型、F3+T 也重载不了 jar 内资源,**必须换新构建 jar 并重启**;这同时解释了之前两条命令无效的另一半原因(改动全都要新 jar)。
- 实机盯:换新 jar 后第一人称持巨阙=刀从右下斜指左上、占右下象限不挡准星;挥砍/格挡跟手无鬼畜;F5=竖持微前倾约 1.2 格握柄在手;剑士武器同验两视角;GUI 图标与扔地上尺寸不变;强化染色照常。

## m399 镇魂/噬魂杖 display 修复=「整屏大刀」真·收尾(作者「还是不行」回修,2026-07-31)
- **五轮才破的案,根因一句话:作者玩的是肉盾,武器叫「镇魂」(class_weapon_tank),而 m398 修的是巨阙(warrior)和流光(swordsman)——之前对话把「肉盾的武器」误认成巨阙,修对了病、开错了刀**。作者换不换 jar 都不可能好,「还是不行」完全成立,与更新流程无关。
- **证据**:重建投影预览器喂镇魂旧值 FP[90,0,80] t[-8.5,9,2.5] ×1.1 → 逐像素复现作者截图的整屏斜贯巨刃(docs/hud/m399_tank_fp_bug_repro.png);同管线回放巨阙 m398 修复值与入库基准 m398_fp_fixed.png 一致=管线校准通过。物品为普通 Item+ColorProvider(染色随强化)=标准 JSON 模型渲染,display 生效,链路闭合。
- **修法(m398 同管线,零盲猜)**:镇魂刃沿 **Y 轴**(15.2 单位,巨阙沿 Z)——等效旋转=巨阙修复姿态右乘 Rx(90°),数值分解**零误差**得 FP[155,0,-25]×0.85、TP[165,15,0]×0.9;平移走「握柄点归位」t=target−R(S(grip)),镇魂握柄=底部立杆中心 (8.57,2.8,8.06),target 沿用巨阙反解出的手锚 FP(0,0.02,0.02)/TP(0,0.05,0.02);左手 y/z 角与 tx 取负镜像。
- **顺手体检其余三把职业武器**(同预览器):鹰扬(summoner)正常、影刺(assassin)可用(占右下不挡准星,暂不动);**噬魂杖(warlock)同族错值反向病**=FP[80,0,90] 把 36.6 单位杖几乎整根垂出屏幕下缘,第一人称近乎空手。同法修:杖头在 -Z 端(z 截面展开度定端),旋转=巨阙姿态右乘 Ry(180) 分解 FP[-115,-25,-180]×0.45、TP[-105,0,165]×0.55,握柄=细杆中段 (8,0,17)。
- **预览器本体入库 tools/display_previewer.py**(m398 只入了产物图没入脚本,本次补上):管线=v/16−0.5→S→R(向量先Z后Y后X)→T/16→FP 手锚(±0.56,−0.52,−0.72)+FOV70;自带 --grip/--target 握柄归位求平移。后续任何武器 display 调整先出图再落值。
- 修复前后预览四张入 docs/hud(m399_tank_fp_bug_repro / tank_fp_fixed / tank_tp_fixed / warlock_fp_fixed)均已复核。纯资源变更,零代码零配置,**configVersion 仍 139**;两预检仅 missing pack.mcmeta 已知假阳性。
- 实机盯:换新 jar 后第一人称持镇魂=刀在右下象限斜指左上不挡准星、F5=竖持微前倾握柄在手;术士拿噬魂杖第一人称能看见杖且杖头朝上;巨阙/流光维持 m398 观感;影刺/鹰扬不变;强化染色照常;左手模式镜像正常。

## m400 四职业武器 display 全量回退原始值(作者点名:「镇魂是肉盾的 巨阙是战士的 噬魂杖是术士的 流光是剑客的,这几个怎么都给我改了 都不对的」,2026-07-31)
- **定性:m398/m399 是误改,本轮全部撤销**。四把职业武器(镇魂 tank/巨阙 warrior/噬魂杖 warlock/流光 swordsman)的 display 原本就是建模者按各自模型逐把手调的正确值,不是 bug;m398/m399 的投影预览器管线看似严谨,实际是把好值当病灶——「整屏大刀」的真凶早在 **m396(第一人称站姿隔离)+ m397(站姿默认关)** 就已修掉(m260 站姿循环动画把武器横持脸前),display 从头到尾是无辜的。m398「逐像素复现」复现的是「原值+站姿横持」叠加态,站姿关掉后原值本就正常,复现即误导。
- **修法**:`git checkout a8f7548(m397) -- 四个模型 json` 整文件回退(display 连同原始格式一并恢复,几何/UV/tintindex 校验无损:流光252/镇魂286/噬魂杖347/巨阙374 elements)。恢复后原值=流光 FP[90,0,80]×0.9、镇魂 FP[90,0,80]×1.1、噬魂杖 FP[80,0,90]×0.8、巨阙 FP[85,-5,80]×0.6(TP 各自原值同回)。
- **教训入册(铁律级)**:①**改任何一把武器前先问作者「你玩的哪个职业、手里哪把」**,职业↔武器对照表固定=肉盾·镇魂/战士·巨阙/术士·噬魂杖/剑客·流光/刺客·影刺/召唤师·鹰扬,写死在此别再认错;②**「顺手体检并修改」其它武器=禁止动作**——m399 顺手改噬魂杖就是本次事故的一半,作者没点名的文件一个都不许碰;③display 手调值默认视为建模者意图,预览器只做参考,与实机截图矛盾时以作者实机为准。
- 纯资源回退,零代码零配置,**configVersion 仍 139**;tools/display_previewer.py 与 docs/hud 预览图保留(仅供参考不再作为改值依据);预检两条 missing pack.mcmeta=已知假阳性。
- 实机盯:换新 jar 后四职业各拿本职武器,第一/第三人称观感应回到 m397 及以前的原始状态;若届时仍有某一把观感不对,**只报那一把、只改那一把**。

## m401 流光第三人称持握修复(作者截图:m400 还原后刀横浮在腰上没握手里,2026-07-31)
- **事实修正(m400 说满了的话收回)**:m400 断言「原始 display=建模者手调的正确值」——对第三人称不成立。流光原始 TP[90,0,90] t[-9.75,8.5,2] ×1.2 的平移把刀挪出手点半格多,实机=刀横贯腰前悬浮、不在手里;这观感从建模入库起一直如此,此前被第一人称习惯/站姿动画等叠加因素掩着没单独暴露。m398/m399 仍定性误改(方向也不对),但「还原=正确」同样不成立,两笔都入册。
- **本轮方法(不再用预览器猜绝对姿态)**:以**原版剑 item/handheld 的 TP display(rot[0,-90,55] t[0,4,0.5] ×0.85,游戏内天然正确)为锚点**做相对求解——量出流光真实几何(握柄=细杆段中心 (15.34,0,0.55),刀尖 (0.9,0,15.01),全长 20.4 单位,刃弦 45° 斜置与原版剑同布局),构造基变换 Q 把流光的(刃弦,平面法向)对齐到原版剑的(刃向,法向),R=R_vanilla·Q 分解得**整数解 rot[-145,0,90]**;scale=15/20.4≈**0.73**(视觉刃长≈原版剑×1.2 的武士刀比例);平移走握柄归位 t=target−R(S(grip)),target=原版剑握柄在手部空间的落点=**[-5.84,6.06,0.08]**。数值终验:流光与原版剑映射后**刃向、握柄落点逐位重合**(误差<0.006)——该法不依赖第三人称渲染管线的任何未知前置变换,原版剑在游戏里怎么正确,流光就怎么正确。左手镜像 rot[-145,0,-90] t[5.84,6.06,0.08] 同回。
- **改动面**:仅 class_weapon_swordsman.json 的 thirdperson_righthand/lefthand 两小节(64 行文本替换),FP/GUI/ground/head/fixed/几何全部不动;其余三把武器(镇魂/巨阙/噬魂杖)一个字未碰=守 m400 铁律「只报哪把改哪把」。纯资源零代码零配置,**configVersion 仍 139**;预检两条 missing pack.mcmeta=已知假阳性。
- **预期观感**:换新 jar 后第三人称持流光=握柄在手、刀尖斜指上前方(与原版剑同款持法)、刃长约 1 格出头;第一人称不变。
- 实机盯:F5 前后左右各看一眼(握手里/朝向/大小),左手模式一眼;如果作者觉得刀该更长/更斜,报个感觉(如「再长两成」「再立一点」)——现在锚点法在手,单参数微调一轮就位,不会再跑偏。

## m402 镇魂接回原作者 display + 流光撤销 m401(作者供原始模型包+点名「这是镇魂 你别改错了」,2026-08-01)
- **认错对象二次事故**:m401 把作者截图那把认成流光并改了流光——错了,**作者玩肉盾、截图从头到尾都是镇魇(m399 当年认对了武器,m400/m401 反而丢了这个正确结论)**。热栏里多把武器并存,靠图标猜=不可靠,这次作者亲口点名+供包,身份坐死。
- **真相闭环(作者上传原始模型包 Voxel Models Pack「恶意检察官」)**:包内 sword_1 与仓库 class_weapon_tank **286 元素 from/to 逐一相等**=镇魂的出处;包自带原作者游戏内验证过的 display(TP[0,90,-3] t[0,12.5,2] ×2.8 / FP[0,89,0] t[0,9,3] ×1.5,左手镜像),与仓库值(TP[90,0,90]/FP[90,0,80]…)完全对不上——**镇魂当年装入仓库时四个手持小节被人手调坏,gui/ground/fixed 三节倒是原样保留**。m400「还原原始值」还原的是装入时就坏的值,所以作者换 jar 后「还是一样」完全成立。
- **修①镇魂**:四个手持小节(TP/FP×左右手)逐位替换为包原作者值,gui/ground/fixed 本就一致不动,几何/tintindex/贴图引用零改动(58 行);写入后与包 display 逐位一致校验 True。
- **修②流光**:m401 属未经报修的错对象改动,整文件 `git checkout 1b7b8ed` 撤回 m400 原状(m401 的锚点法数学本身无错,存档进 DEVLOG 备查,但按「只报哪把改哪把」铁律撤)。
- **铁律追加**:武器身份只认两种证据=作者亲口点名 / 作者供的原始包几何指纹比对;热栏图标、界面截图一律不作为身份依据。**装入第三方模型时 display 必须原样保留原包值,禁止手调**——镇魂这口锅就是当年装入手调埋的。
- 纯资源零代码零配置,**configVersion 仍 139**;预检 missing pack.mcmeta=已知假阳性。
- 实机盯:换新 jar 后持镇魂第一人称=刀在手中不再整屏横贯,F5 第三人称=握手里(原作者姿态,×2.8 大剑观感);流光回 m400 观感;其余武器不变。若镇魂观感仍有偏差,只微调镇魂并以包值为基准报差量。

## m403 技能CD HUD 无框半透明+六档停靠(作者:「这个技能位置不好 放在别的位置 不能设计成方形的 可以半透明」,2026-08-01)
- **对象**=m346/m353 的技能CD常显([R]混沌斩/[G]深渊吞噬/[V]终焉降临/[X]大招/[C]小技能),旧观感=玻璃芯片方框钉在血条面板左侧外(w/2-180),作者嫌位置差+方框丑。
- **①无框半透明(新默认 skillCdHudPlain=true)**:整行改纯文字带阴影——无底、无描边、无高光、无整面闪;透明度 skillCdHudAlpha(默 165,40~255 钳制)打进文字 ARGB 高位;冷却进度从"芯片内充能填充"改**文字下 1px 细线**从左涨满(非方框);转好瞬间从"金框闪"改**整行文字向白亮闪 12t**(mixColor 在树);未解锁行透明度再×0.6 弱化存在感。skillCdHudPlain=false=逐行回 m353 玻璃芯片旧观感(代码原样保留)。
- **②六档停靠(skillCdHudAnchor,照 m308 看板套路)**:0=右下(新默认,右缘 w-8 热栏上方 h-44 向上堆)/1=左下/2=右中/3=左中/4=右上/5=左上/6=面板左侧(m353 旧位);左停靠自动左对齐(drawSkillCdRow 签名 right→edge+alignLeft,芯片版左停靠换算等效右缘),顶部停靠先算总行数(职业2+武器技能数)把起始行压下来防顶裁;skillCdHudOffsetX/Y 微调照旧叠加,终值钳回屏内乱填不飞屏(m308 同口径);行高无框13/芯片16。
- **③设置屏**:「界面·HUD」技能CD区 +12 钮=样式两态/停靠七档/透明三档,即点即改(HUD 每帧读配置)。
- 配置+3,**configVersion 139→140**(新增字段 GSON 缺键取默认,零迁移);括号自检三文件全平;零新 API 面(fill/drawTextWithShadow/switch箭头/mixColor 全在树)——CI 编译回归即可。
- 实机盯:默认=屏幕右下角热栏上方五行半透明小字(不挡准星不压面板);冷却时看橙秒+文字下细蓝线涨满、转好看整行白闪;设置屏切「左下/右中」看落位与左对齐、切「玻璃芯片」回旧观感、透明三档对比;GUI 缩放 2/3/4 各看一眼不顶裁。

## m404 设置屏「数值微调」页=滑动条+数值直输(作者:「UI 的那些调整都可以输入数值或者滑动条来调整」,2026-08-01)
- **VisualFxScreen 新增第 4 页签「数值微调」**:20 个 UI/观感数值项每项一对控件=**滑动条**(拖动实时刷标签内嵌当前值,**松手/键盘步进才发 config set**——防拖动期间每像素刷一条命令+写盘)+**输入框**(回车=解析→钳量程→精确设置,乱输回显当前值;主键盘 257/小键盘 335 双回车码);滑条推值自动回写配对输入框,输入框应用后 clearAndInit 重建让滑条跳到新位置。
- **项目清单(三分区,字段名与 int/double 类型已逐一 grep 核实)**:HUD=CD横移/纵移(±300)/CD透明(40~255)/看板横移/纵移(±300)/血条缩放(0.3~1.5);镜头特效=打击震动/FOV冲击/顿帧(0~2)/登场震(0~3)/转场强度/飘字大小/夜尘密度(0~2)/夜声音量(0~1);刀光姿态背挂=刀光大小/姿态幅度(0.5~2)/背挂角度(±180)/背挂大小(0.3~1.5)/背挂下移/贴背(0~1)。原三页预设按钮全保留=快捷档,数值页=精调,两不冲突。
- **值读写口径**:读=反射 YongyeConfig.get() 字段(单机/局域网同 JVM,config set 改的就是这个对象读回即最新);写=先本地反射回写(int 四舍五入/double 直设)再发 config set 走服务端权威+落盘,双写同值幂等——本地先写是为了 clearAndInit 立即重建时不吃命令异步一拍的旧值。格式化统一 Locale.ROOT 防区域逗号(m188 教训)。
- **待编译验证(低险)**:SliderWidget 仓库首用——构造器 (x,y,w,h,Text,value) 与抽象 applyValue()/updateMessage()/私有 setValue 已拉 yarn 1.21.1 官方映射逐字核对,onRelease/keyPressed 为 ClickableWidget 标准覆盖;TextFieldWidget 在树(爆率编辑器先例);Screen.keyPressed 覆盖为标准 GuiEventListener 方法。报错只会在 CfgSlider 那几行。
- 纯客户端零新配置字段,**configVersion 仍 140**;括号自检 68/68·458/458 全平。
- 实机盯:背包→设置→「数值微调」页,拖 CD横移 看技能CD实时挪+松手落值、输入框敲 -120 回车看滑条跳位、血条缩放拖 0.5 看 BOSS 血条变小、乱输 abc 回车看回显不炸;切回其它页签预设按钮照旧;GUI 缩放 3/4 看两列排布不挤压。

## m405 技能CD改「方块CD·仅冷却中显示」(作者供草图定稿+纠m403误读,2026-08-01)
- **需求勘误入册**:m403 把作者「不能设计成方形的」读成"禁止方框"做了文字列表——作者本意是反问「不能设计成方形的(格子)吗」,即**要 MMO 式正方形技能格**。本轮作者供草图(面板/阶段名上方居中一排五个方格)+明确口径:「CD 开始计时才显示,没有在 CD 就不显示」。
- **实现(样式0=新默认)**:skillCdHudPlain 布尔升级为 **skillCdHudStyle 三态**(0=方块CD 新默认/1=m403 无框文字列表/2=m353 玻璃芯片列表,后两者仍常显+吃 skillCdHudAnchor 停靠);方块样式=22px 正方形一排,槽序 R G V(持武器且已解锁,混沌/龙魂免解锁照 m331)+大招+小技能(有职业),**固定槽位不重排**(照草图预留,某格转好消失其余不挪);水平居中于屏中,基准 y=h-112(避开面板 h-55/阶段名 h-66/箭头 h-82/action bar h-86),skillCdHudOffsetX/Y 照旧微调(m404 滑条页直接拖),钳屏内。
- **格内观感**:冷却中=半透明暗底(alpha=skillCdHudAlpha×0.75)+蓝灰描边+**自下而上蓝色恢复填充**(峰值当分母)+上行键位金字(长键名截两字符)+下行剩余秒橙字(>999 封显);**CD 结束=绿框绿✔按 readyFlash 12t 淡出随即彻底消失**;不在 CD=完全不画(作者定稿口径)。描边=fill 四边照 comboRing 手法,全 fill/drawText 在树**零新 API 面**。
...
## m410 天赋加点脉冲(路线图21,2026-08-01)
- **确认制触发(不空欢喜)**:点击可点节点只"挂起"(记 id+当前级+时刻)照旧发 TalentLearnPayload;渲染帧看到 TalentSyncPayload 把该节点等级**真涨了**才播脉冲,服务端拒绝(竞态/点数被花)=挂起 1.5s 过期无事发生;**天赋数值/树结构零碰**(路线图红线)。
...
## m414 干弟小队设置屏(作者点名:「在UI设置里弄,只有召唤师职业才能看见」,2026-08-01)
- **新 GanDiConfigScreen**:五行=五只,每行 名字框+皮肤ID框+单行「应用」,底部 全部应用/还原默认/返回背包;开屏预填当前配置(同 JVM 直读);顶部提示"留空=默认、皮肤几秒生效";输入框写法照爆率编辑器(m301 在树),行首黄字标默认名。
- **入口只对召唤师可见**:背包左侧外列尾追「干弟」钮,门=`"召唤师".equals(ClientStats.className)`(当前激活职业,换职业开关背包即时增减);不吃 featureUnlocked——有职业本身就是解锁凭证。
- **落值复用 m413 命令后端**(服务端权威写配置+名字在场即改+皮肤客户端下一帧自动换):空框以 **"-" 哨兵**下发,puppetSet 收 "-" 清槽回默认(命令行手打 `-` 同效),反馈里空槽显示"默认";名字/ID 在 UI 侧剥逗号与空格、服务端同口径再兜一层。
- 纯客户端 UI 零新配置,**configVersion 仍 148**;三文件括号平+javac 语法筛查过;命令入口保留(UI 与命令共后端两不废)。
- 实机盯:召唤师开背包见「干弟」钮、切别的职业钮消失;点开填名字+正版ID点应用→在场干弟即改名、几秒换肤;留空应用=回默认;全部应用/还原默认一键;非法ID不换肤不炸。

## m415 战斗日志简版(路线图24,2026-08-01)
- **三路来源**:①承伤=新 S2C CombatLogPayload(来源名≤12字+伤量;enableCombatLog 关=服务端零发包,与受击方向指示同挂点共用其预算闸=天然限流);②输出=DamageNumberPayload 客户端就地取材**只记暴击/处决**(DoD 口径「暴击/处决」,普通命中不进防刷屏);③异常状态=每 tick 自扫新增 HARMFUL 效果(getCategory 链照 ArtifactManager 在树写法,去重键=效果名称串不赌 RegistryEntry 新 API),记 名称+等级+时长。
- **观感**:右缘热栏上方(h-96 向上,与技能CD方块居中错层)右对齐小字列,新条目顶入、8s 淡出;配色 承伤红/暴击橙/处决暗红/状态紫;行数 combatLogLines(3~12,数值页滑条)。
- 配置+2(默认关+行数 6),**configVersion 148→149**;设置屏开关两钮+日志行数滑条;七文件括号平+javac 语法筛查过;新协议 1 包(source 非空兜底踩坑第 5 条)。
- 实机盯:开日志被怪打看「← 僵尸 -234」、天赋暴击看「→ xx 1.2K 暴」、处决看「→ xx 3.4K 斩」、中毒看「☠ 中毒 II 10s」;普通命中不进日志;关配置整列即消;拖行数滑条上限即变。

## m416 普通怪百分比附伤默认去除(作者点名:「怪物打人太痛了,精英以上可以这样,普通怪太强」,2026-08-01)
- **根因**:高血量反制(HighHpCounterHandler)的门是 `!boss && !elite && nf < 1` ——**永夜≥1 后普通怪也吃"精英半额"附加**(百分比 elitePercentDamage×0.5=0.5% 最大生命 + 真伤 eliteTrueDamage×0.5=1.0,魔法伤无视护甲)。作者 1.1B 血 → 普通怪一刀白送 550 万,越肉越痛,与"高血量反制该反制的是精英/BOSS"的本职跑偏。
- **修(三态门,新字段 normalMobPercentDamageMode 默认 0)**:0=**关(新默认)**普通怪只打原版面板伤;1=高难度才开(地狱/深渊/永夜难度且已入永夜——沿用 m251 高难门 growthSuppressionOn 口径——附加精英半额);2=旧行为(永夜≥1 即附加)。**精英/BOSS 不过此门照旧全额**,禁疗/重创逻辑不动。
- 新字段走 GSON 缺键默认=0,老存档自动获得新行为,无需版本门迁移;**configVersion 149→150**;两文件括号平+javac 筛查过。想回旧手感:`/yongye config set normalMobPercentDamageMode 2`。
- 实机盯:永夜 I 让僵尸打一下,掉血应≈原版面板伤(不再上百万);精英(幽蓝魂火)/BOSS 打人仍带附加;设 1 后在游玩/困难难度普通怪无附加、切地狱+入夜恢复半额。

## m417 色盲/低刺激模式(路线图25,2026-08-01)
- **低刺激整档(lowStimulusMode 默认关)**:FxBudget 添三中枢——`flashScale()`(低刺激×0.25/弱闪光×0.5/常规×1,**替代各处手写 reduceScreenFlash 三目**)、`motionScale()`(震屏/FOV/顿帧 ×0.3,在 CombatFxManager.onFx 入口统一缩一次全通道生效)、`pulseOn()`(CD 转好闪/加点脉冲/标题呼吸三类脉冲装饰整体关,标题淡入保留)。八处消费点换接中枢:起手光晕/强化红闪/讨伐金闪/永夜转场罩/濒死暗纱(两处)/打击动感/CD 闪/加点脉冲/标题呼吸;m408 挂账的「标题呼吸收编低刺激档」一并清账。两开关同开取更低,各专属开关照常独立。
- **色盲记号(enableColorblindMarks 默认关)**:品质不只靠颜色——tooltip 品质行加 **◆×档位**(数量=WeaponQuality 枚举序+1,封 8);拾取卡加 **□/▶/◆ 三档形状前缀**(BMP 安全集,踩坑第 9 条口径);暴击/处决飘字本就带「暴/斩」缀字、光柱三档本就宽窄有别(0.16/0.20)——两处天然合规记进 DoD 不再动。
- 配置+2,**configVersion 150→151**;设置屏低刺激/色盲记号四钮;十二文件括号平+javac 筛查过零新 API。
- 实机盯:开低刺激后放大招/强化碎裂/杀 BOSS/永夜升级四类闪光明显变淡、挨打震屏顿帧变轻、CD 转好不闪、标题不呼吸;开色盲记号看 tooltip ◆ 串与拾取卡形状前缀;两开关关掉逐帧回原样。

## m418 HUD 安全区(路线图26,2026-08-01)
- **安全边距中枢**:FxBudget 添 safeX()/safeY()(hudSafeMarginX/Y,0~80 钳制,默认 0=现状零变)——超宽屏/曲面屏/直播遮挡场景把**边缘停靠**的 HUD 统一向内收:看板六锚位(左右缘±X、上下缘±Y,居中锚不吃)、技能CD(方块基准 y 与列表七档停靠边缘)、战斗日志(右缘 X+基准 Y)、拾取卡(右缘 X)、FX 调试面板(左缘 X+底 Y);**居中元素(玩家面板/阶段名/转场字幕/CD方块横向)不受影响**——安全区治的是"贴边",不是"居中"。
...
## m422 设置屏/调试菜单整页滚动(作者点名:「设置显示不全,应该自动识别界面尺寸 或者做一个滚动条」,2026-08-02)
- **根因**:VisualFxScreen / DebugScreen 的 init() 从 tabY+tabH+8 一路向下摆、零溢出处理——GUI 缩放大(逻辑高矮)时内容直接冲出屏底;更糟的是返回/关闭钮用 `Math.min(height-24, y+6)` 钳底,超屏时**浮盖在内容按钮上**(作者截图里"返回背包"压住主菜单动效行即此)。
- **方案:新 ScreenScroller 共用整页滚动器,两屏同修**——标题·页签·按钮·滑条·输入框·返回钮**全部登记随滚**(没有固定元素=天然互不遮挡,不需要 scissor 裁剪,超出窗口顶/底由窗口边缘自然裁掉);内容装得下时 maxScroll=0 整体休眠,行为与旧版零差别,即"自动识别界面尺寸"与"滚动条"一次给齐。
- **三种滚法**:滚轮(每格 42px≈两行,先让 super 分发给子控件没人吃再整页滚)/ 拖右缘拖块(点击判定带 10px 比可见轨道 4px 宽好点中,拖动中金色高亮)/ 点轨道跳转(拖块中心对齐鼠标)。拖块高=视口占内容比、封 20px 最小;仅溢出时才画条。
...
## m425 携带镜像不再复制攻速——修「拿工具第一人称隐形」(作者实机截图,2026-08-02)
- **现象**:拿职业武器第一人称能看见手上武器,切成下界合金镐等工具后**手和物品整个消失**;作者远在 m146 时代就报过一次「镐子不显示」,当时审计只查了渲染面(无镐注册/无模型覆盖/无物品渲染 mixin)误判"非本 mod、疑外部资源包"——**悬案本轮告破,病根在属性面不在渲染面**。
- **根因链**:m133 携带即生效(PlayerUpkeepHandler.updateCarry)把背包职业武器的 MAINHAND 修饰**全量**镜像到玩家,包含大额**负攻速**(鹰扬 -2.6/龙魂 -3.0 等)。手持工具时工具自身负攻速照常生效 → 两份叠加(4-2.8-2.6=-1.4)被属性下限钳到 **0** → 攻击冷却永远充不满 → 原版 HeldItemRenderer 第一人称抬手动画的目标值 = 攻击冷却进度**立方** ≈ 0 → 手持物永远压在抬起动画最低点 = **隐形**(顺带只能打出 0% 蓄力挠痒刀)。拿职业武器时镜像本就撤销、攻速只吃武器自己那份 → 可见,与作者「拿武器能看见、拿工具看不见」逐字对上。
- **修**:镜像循环里**跳过 GENERIC_ATTACK_SPEED**——攻速是"这把武器抡起来多快"的持械手感,不是"带着武器人变快/慢"的携带增益,镜像它本身就语义不通;攻击/生命/护甲等携带增益照旧。一行 continue+注释,零新 API(RegistryEntry 常量恒等比较,类型与 Entry.attribute() 一致);零配置,**configVersion 仍 155**。
- 实机盯:背包带职业武器、手持下界合金镐,第一人称镐子应正常可见、挥砍蓄力条正常回充;切拿职业武器攻速照旧(仍吃武器自身负攻速);携带面板攻击加成不变。

## m426 召唤师职业整体下架(作者点名:选职隐藏/设置隐藏/武器隐藏/不再掉落,2026-08-02)
- **做成开关不做删除**:新 `enableSummonerClass`(**默认 false=下架**),一条命令可整体恢复,不砸任何东西;注册面(物品/实体/皮肤/网络包)全保留——**已学召唤师的存档完全不受影响**(技能/傀儡/干弟召唤照常),下架收的是「获取渠道 + 玩家可见入口」。
- **下架清单(开关关时)**:①选职界面无召唤师页签(ClassSelectScreen 过滤;SKILL_INTRO 改按 ordinal 取行防过滤后错位);②服务端权威三门=chooseStartingClass / learn / replaceClass 头部拒绝(选职界面不显示只是 UI,伪造裸包/老档残留召唤师书都被 C2S 复核挡下,铁律口径);③掉落与宝箱=精英职业书池、精英/BOSS/传说宝箱武器池全换新 `ModItems.droppableClasses()/droppableWeaponClasses()`(调用时实时读配置即时生效);④创造栏召唤师书+鹰扬武器不再列出(注册期取一次,改配置需重启);⑤背包「干弟」设置入口随开关隐藏(m423 的 id 修复保留,开关开时逻辑照 m423)。
- 调试菜单「职业」页 +2 钮(开放/下架);配置 +1,**configVersion 155→156**(GSON 缺键默认 false=老存档自动下架,符合点名意图;恢复 `/yongye config set enableSummonerClass true`)。
- 十文件括号平衡自检过;零新 API(全是过滤与早退)。
- 实机盯:新档开局选职只有 6 个页签(无召唤师)且介绍文案与页签对得上;打精英/BOSS/开传说箱刷不出召唤师书与鹰扬;创造栏搜不到二者;老召唤师号进服傀儡/干弟照常但背包无「干弟」钮;调试菜单点「召唤师:开放」后选职页签+干弟钮即回、重启后创造栏也回。

## m427 FX 统计细粒度打点收口(m411 挂账「光柱/血条细粒度丢弃待 24/27 顺路补」——24/27 都过完了没人补,本轮清账;2026-08-02)
- **背景**:m411 落地 FxStats 时,飘字/拾取卡/音效(m419 补)有完整 used/dropped 打点,光柱/血条/叠层只有存活探针(liveCount/isPlaying),面板那三项看不到「每秒新增/被裁多少」——挂账原话「细粒度丢弃待 24/27 顺路补」,m415(24)/m419(27)都收工了没顺上,POLISH_ROADMAP 第 23 项说明也停在过期表述。
- **光柱(LootBeamManager)**:扫描是每 10t 全量重建,不能每轮都计否则每根柱每秒虚报 2 次——新增 PREV_IDS 基线集(上一轮在场实体 id),**只有新出现的柱计 used**、延续在场不重复计;**够格却被同屏上限裁掉的计 dropped**(原来到上限直接 break,改为继续遍历只计数——遍历成本与上限前同级,仍 10t 一次);功能关闭/预算 OFF 时清基线,重开后全按新柱计。
- **血条(MobHealthBarManager)**:onHit 三点位——功能关/预算 OFF 收到命中信号=dropped(与飘字 m411 口径逐字一致);上限挤掉最久未命中条=dropped;**新条建档计 used、老条刷新不计**(条没变多,语义=「新增一个该类特效」)。
- **叠层(三处全屏演出)**:NightfallTransitionFx(onLevelSync+testPlay)/BossKillFx/UltimateCastFx 起播点统一三态——配置关/预算 OFF/强度 0 拦下=dropped;**替换掉进行中的旧演出=旧的记 dropped**(性能预算规则「同屏最多 1 个,新来替换旧的」第一次有了数据面);起播成功=used。
- **面板(FxDebugHud)**:原来光柱/血条/叠层挤一行只有存活数,拆成三行与飘字/拾取卡同格式「活N +x/s 丢y/s」,面板自动按行数/最宽行撑高撑宽无需另调。
...
## m429 自动强化卷接仓库直供(m359 范围说明「自动卷轴如需接同套下轮点名」清账,2026-08-02)
- **背景**:m359 给背包「强化」一键强化接了材料仓库直供,范围说明写明自动卷轴(AutoScrollHandler)是实体槽位之外唯一没接的强化入口、「如需接同套下轮点名」——本轮接上,四强化入口里可直供的两个(背包一键/自动卷轴)口径终于一致。
- **实现(m359 分账语义逐字原样)**:autoEnhance 深扫背包之后并入仓库账——传统材料并入 budget 且**此刻即从仓库扣**(碎裂不退,与背包老规矩一致);强化石并入 direct、键先记下,**成功后才从仓库删**(m302 碎裂不消耗口径);饱和乘防溢出(m293 口径);无论成败 VaultManager.sync 发回最新快照(仓库界面开着立即刷新)。
- **仓库高档石同受档位闸**:autoScrollMaxStoneTier 对仓库里的强化石同样生效(> 档位的留在仓库不动)——亿级石该由玩家亲手决定砸哪件,自动化不解锁任何超模路径(m302 立场不变)。
...
## m431 搜集任务接仓库(m358 遗留②清账)——顺带修出一个「抽到必败」的实打实 bug(2026-08-02)
- **先说 bug**:GATHER_POOL 里有 `ModItems.LIFE_SHARD`(生命碎片),它是**可入库材料**;m357 自动入库每 5 秒把背包区 9~35 的可入库物扫进仓库,而搜集任务完成判定写的是 `p.getInventory().count(targetItem)` ——**碎片刚捡到就被自己人扫走,背包数永远够不着目标 = 抽到这条搜集任务必败**(还连带永夜加深)。m358 挂账写的「搜集任务目标物入库+自动上交」当时只当功能缺口记,实际是隐藏故障;两个系统各自都对,是接缝漏的。
- **修法两面夹**:①**判定侧**新增 `gatherHave` = 背包 + 仓库(`VaultManager.countOf` 按键前缀比对求和,技能书各等级行也能求和;溢出钳 int)——仓库里的**算数**,即「自动上交」,不用手动取出来;任务不扣物品(complete 只发奖),所以「算数」就是全部所需语义,无需从仓库扣账。②**观感侧**自动入库对「本人当前搜集任务的目标物」豁免(新 `QuestManager.activeGatherTarget`)——玩家得看着背包里数字往上涨,收走了会以为没进展。
...
## m434 修残影「左右摇头」(作者实机,2026-08-02)
- **现象**:m433 疾跑残影跟着视角左右摆,像在摇头/甩尾。
- **病根一(主因,甩尾)**:m433 把残影钉在**局部**坐标系「身后固定距离」,而局部系是跟着 bodyYaw 转的——转视角时 bodyYaw 追 headYaw 是**滞后且分步**的(直线跑也会来回微调),整条残影就像转盘上的尾巴左右扫。**残影必须留在过去,而 m433 把它绑在了现在**,这是设计层的错不是参数问题。
- **修一(世界锚定)**:新增每玩家轨迹环形缓冲(13 格 Vec3d),**在客户端 tick 里采样**真实位置(不在渲染里采——帧率不固定会让层间距随帧率变);渲染时取 N 帧前那个点,算它相对 `getLerpedPos(tickDelta)` 的世界偏移,再逆变换回局部系画。残影从此待在玩家**真正走过**的地方,转身时是甩在后面而不是跟着头转。
  - 逆变换推导:渲染器已施加 `Ry(180°-bodyYaw)` 与 `scale(-1,-1,1)`,故局部→世界为 `wx=lx·cos+lz·sin, wz=lx·sin-lz·cos, wy=-ly`;该 XZ 矩阵**对称正交且自逆**,所以世界→局部是同一组式子。**自洽性佐证**:代入 lz=1 得世界方向 (sin yaw, −cos yaw) 正是玩家背后,与 m247 背挂已实机验证的约定对上——两条独立路径互证,沙箱跑不了游戏时这是最强的验证手段。
  - 层间距改**按速度换算取第几帧**(spacing ÷ 速度,钳 1~3 tick):跑得快取近的点、慢取远的点,实际间距才稳定,不然慢走时几层糊成一坨。
- **病根二(次因,头真的在摇)**:每层副本都用当前姿态,head 的相对 yaw/pitch 也是当前的——甩鼠标时几个残影的脑袋齐刷刷一起转。**修二**:画残影前把 head/hat 的 yaw/pitch 归零(与躯干同向),画完**原样恢复**(渲染单线程,共享模型必须还回去);残影本就是一团速度虚影,不该有独立视线。
- **API 面**:`Entity.getLerpedPos(F)`=method_30950、`LivingEntity.prevBodyYaw`=field_6220 / `bodyYaw`=field_6283、`BipedEntityModel.hat`=field_3394(head 已在树 SlashPoseMixin)——**均已拉 yarn 1.21.1 官方映射逐字核对**,不列待编译验证。
- 零新配置(v 仍 161,m433 那五项照旧管用);两文件括号平衡自检过,javac 语法筛查过。轨迹表换世界即清、玩家离开视野即移除。
- 实机盯:疾跑时甩鼠标左右看——残影应**留在身后走过的路线上**不再跟着扫,脑袋也不再跟着转;绕圈跑看残影沿弧线拖成一串;急停残影原地淡出不再瞬移;慢速走(非疾跑)不出;联机看别人跑同样正确。

## m435 永夜天气联动(「大工程点单区」最后一项落地,本区清空,2026-08-02)
- **与 m352 永夜天象的分工(防重复,两者不是一回事)**:天象那边是本模组自造的**限时灾害事件**(血月/酸雨/流星雨);本笔一个新事件都不造,只**调原版天气本身的节律**——永夜越深,天越容易下雨、下得越久、越容易升级成雷暴。新 NightfallSkyWeatherHandler 独立挂载,天象在跑时**整体让路**(NightfallWeatherHandler 新增 public `eventActive()`)——酸雨会强改天气,两边对着拧必打架。
- **为什么这算「联动」不只是刷天气**:原版雷暴自带一串玩法后果——天光压暗到亡灵白天不燃、光照判定下调导致刷怪位变多、末影人/苦力怕活动范围变广。把「下雨/打雷的频率」挂到永夜等级上,危险度是靠**原版自己的机制**放大的,本模组一行伤害逻辑都不用碰。**刻意不加自定义受伤/减益**:那是天象那边的职责,重复叠加只会让后期彻底无法出门(m416 刚因为「怪物打人太痛」调过一轮,不能再往同一个坑里加)。
- **雪是免费的**:原版在寒冷群系把降雨渲染/结算成降雪,「雨雪」一条实现两种表现,不需要按群系分支。
- **口径**:只管主世界(下界/末地无天气);**永夜 0 级完全不介入**,原版天气循环原样跑;每 600t(30s)检定一次,起雨概率=每级增量 0.06 × 永夜等级(封顶 0.9),雨时长=4800 + 1200×等级 tick,雷暴需永夜 ≥3(永夜III·围城)再过 0.35 概率;已在下雨时够等级可**升级**成雷暴。
- 配置+7,**configVersion 161→162**;设置屏加「永夜天气联动」区 8 钮(开关/雨频三档/打雷门槛三档);五文件括号平衡自检过,javac 语法筛查过,主类重复注册自检过(铁律)。
- **API**:setWeather/isRaining 与 NightfallWeatherHandler 逐字同款(在树 CI 已编),isThundering 已核 yarn 1.21.1 官方映射 method_8546 —— 零待编译验证。
- **路线图**:大工程点单区最后一项划掉,**本区清空**(疾跑残影 m433 / BOSS 击杀慢动作作者明确不要已撤出 / 天气联动 m435)。
- 实机盯:`/yongye nightfall 1` 后等几轮检定看开始频繁下雨、`nightfall 5` 看几乎一直下且常打雷;永夜 0 时天气与原版无异;酸雨天象触发期间本系统不插手(天象结束后恢复调度);寒冷群系看下的是雪;雷暴天白天出门看亡灵不燃、怪变多(原版行为,即本系统想要的压迫感);关 enableNightfallVanillaWeather 完全回原版。

## m436 残影二次回修:朝向也要凝固在过去(作者实机「还是摇头,能不能头不动」,2026-08-02)
- **m434 只修了「在哪」,没修「朝哪」**——这是我上一笔的漏。位置钉到世界坐标之后,每一层的**朝向**用的仍是渲染当帧的 bodyYaw,于是所有残影跟着当前朝向**齐刷刷原地转**:转视角时整排一起摆;更要命的是**直线跑也摆**——原版 bodyYaw 本来就会朝移动方向来回微调(`tickHeadTurn` 的追随逻辑),这就是作者「明明没怎么转视角也在摇头」的由来。m434 修完主观上「摆得小了点」但没根治,正是因为只解决了平移没解决旋转。
- **修**:轨迹环形缓冲从只存 Vec3d 扩成**位置 + 当时的 bodyYaw**;每层渲染前右乘 `Ry(历史yaw − 当前yaw)` 把它转回被采样时的朝向。
  - 推导:渲染器施加的是 `Ry(180°−当前yaw)·S(-1,-1,1)`,要等效换成 `Ry(180°−历史yaw)·S`,需右乘 `R = S⁻¹·Ry(当前−历史)·S`;而 `S·Ry(θ)·S = Ry(−θ)`(S=diag(-1,-1,1) 自逆),故 **R = Ry(历史yaw − 当前yaw)**。角差过 `MathHelper.wrapDegrees` 防 ±360 绕圈时突然转一大圈。
  - 旋转在 translate **之后**施加,轴心自然落在该层自己的位置上(不是绕玩家转),这点很关键。
- **至此残影三要素全部凝固在过去**:位置(m434 世界锚定)/朝向(m436 历史 yaw)/头部(m434 head+hat 归零,与躯干同向)——作者要的「头不动」现在是字面意义上的不动:甩视角时残影纹丝不动,只随身体真实位移向后拖。肢体摆动(limbAngle)仍用当前值,那是有意的:残影本就是速度虚影,腿部同频摆动看着才像拖影而不是一排静止假人。
...
## m439 召唤师重新上架(作者点名「召唤师隐藏移除」,2026-08-02)
- enableSummonerClass 默认 false→true,并按 **m397 铁律走版本门迁移**(configVersion<165 统一置 true):m426 下架时全体老档都是 false,值判等区分不了「m426 旧默认」与「玩家主动关」,按作者决定统一开一次,之后想关设置里关回即长期生效。
- m426 的整套开关逻辑原样保留(选职页签/服务端三门/掉落池/干弟入口全部跟着开关走),本笔只动默认值+迁移;**创造栏需重启生效**(m426 已知边界:注册期取一次),其余即时。
- **configVersion 164→165**;javac 筛查过。
- 实机盯:老档进服选职界面回到 7 页签、背包「干弟」钮回来(m423 的 id 修复仍在)、精英/BOSS 重新掉召唤师书与鹰扬、重启后创造栏也回;之前手动 set false 过的档同样会被开一次(版本门语义,作者知悉)。

## m440 咏唱台词分层 2.0(作者:「更中二更帅的词,可以是强化到多少级激活后面的新学的」,2026-08-02)
- **成长语义与武器技能对齐**:R/G/V 本就按品质解锁(m131 体系:稀有100/史诗250/神器1000),台词照同一张品质表走——新增 **@N 前缀语法**:「@N 台词」=主手武器 ENHANCE_LEVEL ≥N 才解锁,无前缀常驻;默认阈值 @100 稀有 / @1000 神器 / @2500 至尊。等级读主手组件(m211 染色管线同款,客户端天然同步),空手只出常驻句。
- **抽取偏新**:60% 从已解锁的**最高档**那几句里抽——刚强化上去立刻听得出「学了新词」,老词不废新词有牌面;写坏的 @前缀按常驻句处理不吞词(容错)。
- **七职业默认词全部更中二**(每职业 2 常驻 + 3 分层,全 BMP):剑客到至尊是「斩断星河,不过抬手之间。」、武僧「一拳,碎苍穹。」、刺客「你的名字,已从生死簿上划去。」、坦克「我即城墙,我即终焉之壁。」、术士「万物终焉,由我执笔。」、战士「我之怒火,焚尽万军。」、召唤师保留作者梗「肝痒痒了。」升为 @100 档、至尊「此界生灵,皆为我兵。」。
- 默认值改版走**仅旧默认迁移**(七个字段各自:仍是 m438 旧串才换,自定义台词不动);**configVersion 165→166**;两文件 javac 筛查过。
- 实机盯:+0 武器放大招只出常驻句;强化过 100/1000/2500 各放几次,新词应高频出现;`config set chantSwordsman "@500 自定义词"` 验证语法;m438 旧默认档启动一次自动换新词、自己改过的不动。

## m441 武器设计 Skill 入库 + 全武器动态贴图(作者上传 minecraft-java-weapon-designer-v4.skill:「看能不能用上,可以把现有的武器替换了」,2026-08-02)
- **Skill 评估结论:能用,已入库** `docs/skills/minecraft-java-weapon-designer-v4/`(沙箱会清,先 push 落盘=m161/m244 口径;MANIFEST 声明 contains_purchased_assets=false 可安全入库)。对夜蚀最对口的四章:①路线三 Fabric 模组=本项目;②高复杂度体素武器(复杂度分级/几何统计);③**贴图图集与动画贴图**(本笔直接落地);④战斗时间轴同步表(m255 编排器已同款,验收时可借它的表格式)。
- **「替换现有武器」的判断——本笔刻意不做整套替换**:按 skill 自己的几何统计跑了一遍,现有 6 把职业武器是 252~374 elements 的高复杂度体素模型、display 八视角**已实机调好**;skill 首要规则 4「不伪造已测试」+ display 章「不同轮廓需独立调校」——沙箱看不见渲染,盲换 7 把等于把已验证的美术换成未验证的,是倒退。正确路径=先把 skill 的**动画贴图**章用上(零几何风险,观感立涨),整模替换走「一把试点→作者实机看→再批量」的分期,试点等作者点名哪把先动。
- **动态贴图落地(纯资源零 Java)**:8 张武器贴图全部动起来——**大图(≥512²:刺客/剑客/术士/战士)= 2 帧呼吸**(0.88×/1.14×,frametime 12+interpolate,插值回绕天然三角波=1.2s 完美呼吸,**内存只 ×2**);**小图(≤128²:坦克/鹰扬/混沌之刃/龙魂)= 8 帧斜向流光**(1+0.22·sin(2π(f/8+(x+y)/1.4w)),frametime 3+interpolate,1.2s 循环)。与 m211 染色管线**乘区叠加**:亮度呼吸 × 强化等级染色,越高级的武器呼吸的是越深的颜色。
- **按 skill 的像素内存估算章记账**:新增解码内存合计 ≈33.8MB(四张大图各 8~9MB),图集装配无忧(8192² 上限=268M 像素,本包新增 ~8.8M 像素);_e 后缀贴图未被引用不动;**alpha 逐像素零改动**(m210 铁律,已抽检校验)+ 帧间亮度差已程序验证(帧1>帧0)。回退=`git checkout` 本笔前版本的贴图路径(m210 同口径)。
- 零配置零 Java(**configVersion 仍 166**);validate-resource-pack 过(仅 MOD 场景 pack.mcmeta 已知假阳性)。
- 实机盯:手持任意职业武器静止看刀身 1.2 秒一轮明暗呼吸/流光,GUI 物品栏同样动;强化 +2500 的武器看红色呼吸;帧率无感知差异(重点盯低配);嫌呼吸幅度大/小报一声,改脚本两个系数重跑即可。

## m442 剑客整模替换试点「星穹断歌」+ 全武器备份(作者:「按你的想法先做出一把,以前的武器先备份」,2026-08-02)
- **备份先行(作者点名)**:`docs/backup/weapons_pre_m442/` = m441 收工时(c0f01fd)全部武器资产快照——6 个模型 JSON(**原作者 credit「Done by Pramanix」随文件保留**,skill 版权边界条)+ 全部武器贴图与 mcmeta + 一键还原 README(含更早版本的 git 取法:b995c22 静态原图 / 5bbc7bd 彩色原版)。
- **试点选剑客(m441 提议获准),设计「星穹断歌」大太刀**,按上传 skill 的完整流程走:
  - **几何(126 elements=skill 中等复杂度档,17 带 rotation)**:柄尾宝石+双坠→7 节宽窄交替缠柄→护手中枢+双侧 4 阶阶梯翼+22.5° 翼尖→**20 步阶梯收锋刀身**(半宽 1.55→0.40,skill「小体素阶梯做像素化曲线」条)每步=钢面+双刃口+**贯穿发光核心槽**(上下各凸 0.03 防共面 z-fight)→交替侧符文刻痕→三段锋尖核心出锋→6 片 45° 刀背鳍→3 枚 z 轴 ±22.5° 悬浮晶片。坐标全在 −16..32,JSON 60KB(远低于 skill 1MB 警戒线)。
  - **display 八视角逐字复用旧模型**(已实机验证的参数;新模型同为「沿 +Z 平躺、Y 薄、中心 x8」姿态与旧一致,这是能复用的前提)——skill「不同轮廓需独立调校」的风险被压到最小,实机若持握角度不适再单调。
  - **贴图 128² 灰度图集**(m210「墨黑→钢白」口径:拉丝钢/亮刃口/极亮核心/暗缠柄/晶面/符文八材质区,面 UV 带抖动防平涂)+ **8 帧分强度流光**(核心/符文/宝石 ±34%、刃口/晶片 ±16%、钢面 ±5% 微呼吸,frametime 3+interpolate=1.2s);全脸 tintindex 0 → m211 染色管线照吃,**核心槽=强化色的主秀场**。解码内存 0.5MB,比旧 1024² 动态版**降 16 倍**(skill 像素内存章记账)。
  - **轮廓预览已出**:`docs/weapons/m442_swordsman_preview.png`(俯视 X-Z 投影,m184 预览惯例;z 轴旋转的晶片按未旋画,实机会有扭转)。
- **零 Java 零配置(configVersion 仍 166)**;JSON 合法性/坐标界/材质区对齐已程序自检;validate-resource-pack 过(仅已知 pack.mcmeta 假阳性)。旧模型物品 id/lang/染色/动画贴图管线全部不动=纯资源替换,**不满意一条 README 命令即还原**。
- 实机盯(skill display 章清单):手持第一/第三人称、GUI、地面、展示框、头部六视角逐一看持握角度与出屏;核心槽流光 1.2s 一轮、强化 +2500 看红色核心;与拔刀七式/背挂/拖刀同屏观感;帧率无感知差异。**观感定调后批量其余六把,不合意点名改哪里(长度/宽度/晶片数/核心宽窄都是脚本参数)。**

## m443 剑客试点两处实机回修(作者截图:「这个武器是反的啊,然后强化了颜色没变化」,2026-08-02)

### 病一:武器「反的」——不是反,是**柄段 z 位置错位**
- **诊断**:m442 我只按「刀身沿 +Z 平躺」这一条对齐了旧模型,**没有对齐柄段在 z 上的具体位置**。重新按「厚度/宽度比」逐 z 切片剖旧模型才看清:旧模型 z −2..8 是一条 0.31 厚 × 0.50 宽、每层仅 3~8 元素的**匀质细杆=柄**(长达 10 单位的长柄),z 9..18 才是 0.75~0.88 厚、每层 20~86 元素的**刀身**。(第一次剖分用「离中轴半宽」判读被 45° 旋转件带偏——那些绕 origin z=7.91 旋转的扇形件被我错算成「z=0 处半宽 7.4」,险些得出「旧模型柄在高 z 端」的反向结论;换成厚宽比+元素密度才判准。)
- 而 m442 把柄放在 z −4.2..1.2、护手 1.2..3.0 —— **手握点(旧柄中心 z≈3)正好落在我的护手/刀身根部**,于是柄尾从手的另一侧捅出来,看着就是「握着刀身、柄朝前」= 作者说的「反的」。截图里手边那块宽三角正是我 ±4.9 的护手翼,远端那截深色带纹就是被甩出去的缠柄——完全对得上。
- **修**:柄段逐字对位旧模型 `z −2.0..7.6`(9 节缠柄),护手 7.6..9.4,刀身 9.4..17.4(16 步阶梯),锋尖收在 **18.6**(旧模型末端 18.33,防出屏);顺带把 y 厚收进 **±0.55**(旧模型 ±0.575,那是把极薄的扁刀)、护手翼展从 ±4.9 收到 ±3.6(让刀身当主角)、刀身半宽 1.55→**2.30** 起(上版太像针,现在有大太刀分量)。105 elements。
- 预览已重出并**标出手握点红线**:`docs/weapons/m442_swordsman_preview.png` —— 柄段现在横跨该线,这是这次唯一有意义的验收判据。

### 病二:「强化了颜色没变化」——染色管线是好的,是 m211 的**阈值设计**没给低段反馈
- **先排除接线问题**:新模型 756/756 面全带 tintindex(旧模型 1504/1504,覆盖率一致);ColorProviderRegistry 对 classWeapons() 全注册;provider 忽略 tintIndex 直接返色。**管线没坏**。
- **真因**:m211 写的是 `if (lvl <= start) return 0xFFFFFFFF;`,start 默认 **100**——也就是从 +1 强化到 +99,屏幕上**零反馈**,玩家当然认为染色坏了。这是设计缺口不是 bug,但作者的抱怨完全成立。
- **修(m443 低段爬色)**:0<lvl<start 时给一层冰蓝,饱和度 `weaponTintLowMaxSat × (lvl/start)^0.6`(默上限 0.35=淡淡一层,不抢 +100 之后的正戏;指数 0.6 让前几十级就看得出变化,线性的话 +10 几乎无感);start 之后的主曲线改从低段终点**接着往上爬** `sat = 0.35 + 0.65·min(1,1.25t)`,**起始级处两分支同值 0.3500 已程序验证连续**,不会出现「+99 有色 +100 反而变白」的跳变。关 `weaponTintLowRamp` 逐帧回 m211 老口径。
- 配置+2 **configVersion 166→167**;javac 筛查过,JSON 合法性/坐标界自检过,validate-resource-pack 过(仅已知 pack.mcmeta 假阳性)。
- 实机盯:①持刀看**柄在手里、刃朝前**,六视角(第一/第三人称/GUI/地面/展示框/头部)各扫一眼;②拿把 +10~+50 的武器就该看出淡冰蓝(这次是重点),+100 起按老曲线爬到 +2500 正红;`weaponTintLowRamp false` 回旧观感。

## m444 剑客试点三修:布局整体翻转——柄在高 z(作者二连截图「这两个都是反的」,2026-08-02)
- **m443 的修法方向本身就错了**。这次不再从旧模型几何「猜」,两条独立证据定案:
  ①**作者第三人称截图是铁证**:向斜后上方伸出去的那截带**均匀分节纹理**的杆,正是我的 9 节宽窄交替缠柄(分节纹样在这把武器上是唯一的)——即手抓住的是护手/刀根,柄插在了刀该在的方向;
  ②**旧模型重读**:z 9..15 那团每层 20~86 元素、0.75~0.88 厚的**密集簇才是缠柄**(华丽武器的柄比刃厚是常态),z −3..6 那条 0.5 宽的匀质薄片是刃(彩色贴图已不可得——m426 才换的这版模型,入库时就是灰度;亮度采样 z≤0 段 171 偏亮=钢,支持此读)。m443 把「细=柄」当了判据,**恰好读反**。
- **教训入 SKILL.md 候选**:判读第三方模型「哪头是柄」,几何薄厚不可靠(细的可能是刀,厚的可能是柄),要用**手握处材质纹理**(分节缠绳/皮革)或实机截图对照;程序化模型自检要加「手线材质断言」。
- **修**:布局整体翻转——柄尾 z 15.4..17.0、缠柄 **8.6..15.4**(对位旧密集簇 9..15,手握中心≈z12)、护手 6.6..8.6(翼尖 −22.5° 朝刃侧)、刀身 6.6→−3.2 十六步收锋(**越低 z 越窄**)、锋尖收在 −4.6(旧最远端 −2.33,略长 2 格)、背鳍/晶片全跟着翻。105 elements,display 仍逐字复用旧参数。
- **数值自检代替目检**(沙箱预览图 view 渲染不稳,不赌眼睛):①跨手线 z=12 的元素材质=**纯 GRIP**(断言过);②刀身钢面半宽随 z 降单调收窄:z6.0→2.30,z−3.2→0.45(断言过);③JSON 合法、坐标 −4.6..17.0 全在界内、validate 过(仅 pack.mcmeta 假阳性)。
- **顺带确认一个好消息**:作者截图里武器已是**淡冰蓝** = m443 低段爬色实机生效,染色链路闭环。
- 零 Java 零配置(v 仍 167)。实机盯:①第三人称=**柄在手里、分节缠柄不再朝天、刃朝挥砍方向**;②第一人称/GUI/地面/展示框/头部五视角;③GUI 图标方向与旧刀一致(共用 display);④若持握角度想再调(刃太长/太短/角度不顺手),报一句,z 布局四个段点全是参数。

## m445 剑客试点 v4:按作者概念图收形(作者:「手柄有点太长,前面有点太宽稍微细一些,再帅一些,根据这个剑客的武器来」+ 附概念图,2026-08-02)
- **m444 方向已确认对**(作者截图里柄在手中、刃朝前),本轮进入纯造型调参,三条意见逐条落:
  ①**柄太长** → 缠柄 6.8→**4.2**(6 节),柄尾环+宝石+尾芒收进 14.0..15.8;**手握中心保持 z≈11.9**(m444 验证过的挂点,不动);
  ②**前面太宽** → 刃半宽 2.30→0.45 改 **1.05→0.26**(细长直刃转晶剑比例),护手中枢 ±1.5→±1.15、翼展 ±3.6→±2.5,晶片 3→2 并收小;
  ③**再帅+照概念图**(细长微弯、晶蓝刃体、黑柄、悬浮光点)→ **微弯**:刃中轴沿弯向作 0.9·p^1.8 弧偏(阶梯体素弯,自检核心中轴根部 x8.01→锋端 x8.75);**晶核主导**:核心槽相对加宽至 0.24 半宽且上下凸 0.035;**单刃语义**:弯内侧亮刃口、外侧窄暗刀脊;**游离辉光粒 5 粒**沿刃上下交替悬浮(对概念图的漂浮光点);贴图 STEEL 基调 190→**218 提亮**(乘法染色下更吃色=低强化就是概念图那种晶蓝)、呼吸振幅改三档 0.36/0.18/0.10(钢面也参与=整刃晶体感)。
- 106 elements;display 仍逐字复用旧参数;数值自检三连过(手线 z12 纯 GRIP / 半宽随 z 降单调 / 中轴单调右移);JSON 合法;validate 过(仅 pack.mcmeta 假阳性)。零 Java 零配置(v 仍 167)。
- 实机盯:①柄短了握持是否顺眼、②刃宽是否到位(嫌还宽报数,半宽首尾两个数直接改)、③微弯观感(嫌弯大/小调 0.9 系数)、④低强化冰蓝是否有概念图那味、⑤GUI 图标。定调后其余六把按各自概念开批量。

## m446 剑客试点 v5:握点二次校准 + 签名蓝双轨配色(作者截图:「拿着的地方不对,配色也不对吧」,2026-08-02)
- **握点**:v4 截图里手落在**柄尾端**、整段缠柄悬在手前方(手到护手之间露出完整分节柄)——反推真实手挂点 **≈z14.5**,不是 m444 起假设的 z12(m444 柄 8.6..15.4 恰好够长盖住了 14.5,所以当时看着对;v4 柄缩短到 14.0 截止,手就落到柄尾了——**柄长问题与握点问题在 m444 里互相掩护,砍柄把它暴露出来**)。修=模型整体 **+2.4 z 平移**(含 rotation origin),平移后总包络 −2.6..18.2 与旧模型 −2.33..18.33 **几乎逐位重合**——两条独立证据互证,这个手点这次是钉死的。柄 12.2..16.4,手前余量 2.3/手后 1.9;数值断言更新为「手线 z=14.5 纯 GRIP 且两侧余量 ≥1.5」。
- **配色**:v4 全脸吃 m211 强化染色,低强化饱和度被 m443 低段爬色压在 ≤0.35 → 整刀灰底淡蓝,出不来概念图的通体晶蓝。修=**双轨**:①晶系(核心/符文/晶片/辉光粒/宝石)**去 tintindex + 贴图烘焙签名色**——晶体剑客蓝(b≈r×3 的青蓝梯)与柄尾暖金,**任何强化等级都保持**,这是这把武器的身份色;②钢面/刃口/柄/护手照旧灰度+tintindex,继续随强化变色(成长反馈不丢)。**高强化观感=红钢包着蓝晶**,比全红更有记忆点。tintindex 面 438/636,晶系豁免已程序验证(CORE 采样 b≫r、STEEL r=g=b、GRIP 近黑三断言过)。柄压暗到 36~58(概念图黑柄)。
- 动画照旧 8 帧 frametime3 interpolate,流光在蓝色通道上跑;validate 过(仅 pack.mcmeta 假阳性);零 Java 零配置(v 仍 167)。
- 实机盯:①手握在缠柄**中段**、柄尾露一小截;②不强化=黑柄+灰钢+**亮蓝晶核**(概念图味),强化后钢面变色而晶核恒蓝;③若手点还差半格,报「偏前/偏后」,平移量一个数的事。

## m447 剑客武器换装作者手绘「苍魂剑」(作者上传 azure_soulblade_v10 bbmodel + 调色板:「这个是我画的剑,里面带动画的,你看看」,2026-08-02)
- **工程判读**:Blockbench free 格式,439 elements **调色板画法**(每面 UV 采样 64² 调色板色块——上传的 palette.png 与内嵌贴图不一致,按作者**单独上传的为准**,名字正好对上工程贴图名);竖立布局(刃 +Y 高 56/柄尾 −15,全长 71);带 6 条**骨骼时间轴动画**(idle 整剑悬浮 + 剑气波/招架反弹/剑气凌空/身法如风/万剑归一,骨骼 sword_root+particles)。
- **转制(free→原版物品模型)**,四个不可绕的适配点:
  ①**坐标系**:竖立→平躺,x'=8+0.294x / y'=0.294z / z'=13.03−0.294y(缩放 0.294 使全长 71→20.9≈旧模型 20.7;**柄中心钉在实测手点 z14.5**——m446 校准的成果直接复用);面向重映射 up→north/down→south/north→down/south→up(调色板平色,面内旋转无关);
  ②**非法旋转**:5 个缠柄 wrap 是 **8° z 轴**旋转,原版只认 0/±22.5/±45 → 贴齐 0°(斜纹变直纹,唯一的保真损失,已如实记录);
  ③**上限校验**:439 元素坐标全部落在 −16..32 ✓,JSON 162KB(skill 1MB 线内);
  ④**颜色语义**:作者彩绘=**全签名色,一个 tintindex 都不加**(2634 面全裸色)——强化染色在这把武器上让位给作者美术;若要强化反馈可后补「只染钢灰面」的选择性方案,等作者点名。
- **display 复用已验证参数**(bbmodel display 为空);刃元素 UV 采样验证=深蓝(28,111,230)→亮蓝(50,180,255)→冰白(151,235,255)渐变,调色板映射正确;手线断言=z14.5 跨线 6 元素全为 handle 系 ✓。贴图为作者原版 64² 静态,**删除旧 mcmeta**(不给作者的画自作主张加流光,要加等点名);particle_* 16 个悬浮光粒元素保留(静态悬浮,本就帅)。
- **关于「里面带动画的」要说实话**:那 6 条是**骨骼时间轴动画,原版 Java 物品模型放不了**(物品模型没有骨骼运行时,需要 GeckoLib/AzureLib 这类库+自定义渲染器)。三个消化路径:①五个技能动画的**命名正好一一对应职业技能**——它们的节奏/角度可以移植成 player-animator 的**玩家挥砍动作**(夜蚀 m254 那套,能放!)——这是我推荐的路径,下一笔就能做;②idle 悬浮那条对应的观感,已有 m433 残影+m441 呼吸类流光可以替位;③真要「武器本体骨骼动画」= GeckoLib 集成,新依赖+物品渲染器重写,是个独立大项,值得单独立项讨论。
- 原工程与调色板已入库 `docs/weapons/source/`(署名保全+沙箱防丢);credit 写明「作者手绘,夜蚀转制」。零 Java 零配置(v 仍 167);validate 过(仅 pack.mcmeta 假阳性)。
- 实机盯:①手握柄中段、刃朝挥砍向(手点是 m446 校准值,应当直接对);②苍魂剑配色=你调色板的原色,蓝刃渐变/棕金柄;③GUI/地面/展示框;④五个技能动画要不要我移植成玩家挥砍动作,点名哪个先来。

## m448 苍魂剑握点三校 + 万剑归一幻影剑群(作者截图:「手柄往后拿一些,现在太靠前了;这动作没有,剑的大招万剑归一没有剑啊」,2026-08-02)
- **握点三校**:截图=手正落在金护手处(≈z12.5),要「往后拿」=让手落在柄段后 1/3 → 模型整体 **−2.6 z**,柄段 9.84..13.96、手点 12.5 落在柄段 65% 处、柄尾+尾坠露在手后。包络 −6.0..14.8 在界内。(握点估值史:m444 假设 12 → m446 估 14.5 → 本次实测回 ≈12.5——m446 那次高估是把 v4 截图「手在柄尾」读过了头;**现在有作者手绘模型的金护手当基准点,12.5 是三轮里证据最硬的**。)
- **万剑归一幻影剑群(新 SwordRainFx,192 行)**:剑客放大招时身后升起幻影剑阵——**渲染的就是玩家手里那把武器的模型**(作者换什么剑、幻影就是什么剑),展扇悬停后分三波朝准星方向俯冲,末段淡出。
  - **编舞时间轴直接取自作者 bbmodel**:「万剑归一」动画 length=2.6s、sword_root 关键帧 0/0.55/1.2/1.75/2.0/2.6 → 0~0.55 升起展扇、0.55 起悬停蓄势(微幅呼吸)、**1.2/1.75/2.0 三个拍点各放一波**(剑按 i%3 分波)、2.6 收场。骨骼动画原版放不了(m447 结论),但**节拍能放**——作者画的是剑的挥舞,这里译成剑群的进军。
  - **零新 API 面**:世界渲染=AFTER_TRANSLUCENT+相机差值(DamageNumberManager 同款);画武器=getItemRenderer().renderItem(FIXED)(m247 背挂逐字同款);满亮度 0xF000F0。纯本地纯演出,伤害仍由服务端大招本体结算,一行游戏逻辑没有;时间驱动到点必消;空手放大招不出阵(没剑可归一)。
  - 阵形:身后 1.6~2.6 格、左右 ±2.8 展扇、外侧略高成弧、扇内偏航 ±14° 微散、三档缩放 0.9/1.05/1.2;俯冲 45° 低头突刺飞 14 格。
- **「这动作没有」另一半**:剑客大招同边沿本地播 m254 七式**第 6 式·居合**(SlashAnimManager.playFor(player,6);V_IAI 常量私有,字面量+注释;第一人称 playFor 自拒走程序化姿态=m396 口径)——蓄而后发的气口配「归一」。
- 配置+2(enableSwordRainFx / swordRainCount 6~22 默 14,FxBudget 缩放),**configVersion 167→168**;四文件括号平衡+javac 筛查过;接线三处 grep 回验。
- 实机盯:①手落柄段后 1/3、柄尾露出(不对再报「前/后+大概几格」);②剑客放大招=居合起手+身后剑阵升起+三波俯冲(注意剑阵用的模型/配色就是苍魂剑本体);③换其他武器放大招看幻影跟着换;④LOW 画质剑数减、OFF 全无;⑤其余职业大招无剑阵(剑客限定)。

## m449 剑客武器还原备份原版(作者:「现在拿的地方更不对了,还不如以前的了,用以前备份的那个武器吧」,2026-08-02)
- **执行还原**:模型/贴图/mcmeta 三件套从 `docs/backup/weapons_pre_m442/` 逐字节还原(md5 三对全等已验证)——回到 Pramanix 原模型 252 elements + m441 二帧呼吸贴图。这就是 m442 备份先行的意义,零损失回退。
- **复盘握点这笔账,认到底**:m442→m448 四轮握点校准(12→14.5→12.5)本质上是**在沙箱里没有渲染地面真值的情况下反复猜手挂点**——每轮截图只能给出「偏了」的定性,给不出模型空间的定量;而旧模型之所以对,是原作者在 Blockbench 里**边看实时预览边调 display 和几何**的成果。上传 skill 的 display 章早就警告过「不同轮廓需独立调校、不伪造已测试」,我在 m442 用「逐字复用旧 display」绕开了警告的前半句,却绕不开根本问题:**display 复用只在几何布局也完全对齐时成立,而"对齐"本身需要视觉验证**。教训写死:今后凡涉及手持物 display/握点,方案只有两种——①作者在 Blockbench 里连 display 一起调好再交给我转制;②沙箱内先做出可信的手点标定(如临时给模型加一个已知坐标的标记块让作者截图读数),标定完成前不动几何。
- **保留的(与模型无关、且各自针对作者点名的问题)**:①m443 染色低段爬色(「强化了颜色没变化」的修复,旧模型 1504 面全带 tintindex 直接受益);②m448 万剑归一幻影剑群+居合起手(「大招没有剑」的修复,渲染的是**手中武器模型**——现在飞的就是 Pramanix 原剑,天然适配);③m447 入库的作者苍魂剑工程与调色板(`docs/weapons/source/`)原样保留,哪天作者想在 Blockbench 里自己调好 display 再上,转制管线现成。
- **武器整模替换试点至此收档**:结论=程序化生成/盲转制在无渲染反馈的环境里做不好「手持物」这一类资产(GUI 图标/地面/展示框这些无握点的视角其实都对了,栽就栽在手持);批量其余六把**不再推进**,除非走上面两条新路径之一。
- 零 Java 零配置(v 仍 168);validate 过(仅 pack.mcmeta 假阳性)。
- 实机盯:剑客武器回到熟悉的原版观感(含 m441 呼吸流光与强化染色);放大招看幻影剑阵飞的是原剑;苍魂剑素材还在仓库里,想续做随时说。

## m450 术士魔法弹:瞬发射线 → 飞行法术球(作者:「远程攻击为什么没有特效,右键蓄力完成后远程法术球是不是应该有啊」,2026-08-02)
- **病根**:m139/m261 的魔法弹是松手当帧的**逐点射线瞬发**,拖尾只有每 ~1.5 格 1 粒的稀疏灵魂火——观感上「什么都没发生」,作者的抱怨完全成立:既叫「弹」,就该看得见一颗弹。
- **修**:新 WarlockBoltHandler,魔法弹变成**一颗真的会飞的球**——服务端 tick 推进(warlockBoltSpeed 格/tick,默 1.5=30 格/秒),球体=灵魂火核心+龙息晕圈+PORTAL 拉丝尾迹,**蓄力倍率越高球越大越浓**(×1≈拳头,×10≈脸盆),满蓄额外 END_ROD 星芒;命中活物/撞墙爆开(SOUL+灵魂火+WITCH,爆点规模随倍率),飞满射程余烬消散;发射有唤魔者吟唱音。
- **玩法口径守恒**:伤害公式/耗血/专属武器加成/蓄力提示**逐字保留在物品侧**(飞前算好随弹携带);命中判定复用旧口径(0.5 步长、1³ 盒、排除玩家不误伤);唯一的玩法变化=「当帧结算」改「球到才结算」——可以打提前量了,这正是「球」的应有之义;嫌慢 `warlockBoltSpeed 8` ≈ 旧瞬发手感。
- **实现纪律**:全服务端 spawnParticles(联机同屏可见,零客户端代码零协议);粒子五种全部在树已用(SOUL_FIRE_FLAME/DRAGON_BREATH/PORTAL/END_ROD/SOUL/WITCH),isAir 撞墙判在树同款(EliteCombatAI/NightfallHordeHandler),END_SERVER_TICK 在树十余处同款——**零新 API 面**;弹道表出界/命中/下线即清,单人同时最多 4 颗防刷屏。
- 配置+1 **configVersion 168→169**;四文件括号平衡+javac 筛查过,fire 接线与主类防重复注册 grep 回验;ClassWeaponItem 旧射线块 47 行 → 6 行转发。
- **顺带答作者「还有什么没做的」——当前全部待办**:①m373~m450 大批 🧪 待作者实机验收转 ✅(转场/战斗/掉落/环境/UI 分组见 POLISH_ROADMAP);②剑客其余四技能动画观感移植(剑气波/招架反弹/剑气凌空/身法如风,照 m448 万剑归一的路子,等点名顺序);③候选:shakeScreensForNearbyPlayers 广播震屏 / 残影四肢冻结开关 / 武器呼吸幅度微调;④苍魂剑若作者在 Blockbench 连 display 调好可随时转制(管线现成)。
- 实机盯:术士右键蓄力(蓄力越久越猛)松手——应看到一颗蓝紫法术球飞出去、命中爆开;半蓄 vs 满蓄对比球径与爆点;打移动怪试提前量;撞墙也爆;`warlockBoltSpeed 8` 试瞬发手感;联机让队友看同屏可见。

## m451 剑客四技能观感移植:作者手绘 bbmodel 的节拍全部落地(作者「继续」自选,2026-08-04)
- **接着 m450 的同一条线索**:m450 治的是「术士远程攻击看不见弹」。同一把尺子量下去,剑客四条技能全有同病——
  **机制在跑、世界里几乎没有表现**:①剑气波打到 4~6 格外,特效只有身前 1.5 格处 6 颗 SWEEP、**连音效都没有**;
  ②剑气凌空 12 格穿透,粒子却是**在同一 tick 里每格画 1 颗**——闪一下就没了;③格挡反击**只有一声盾响、零粒子**,
  分不清「挡住了」还是「挨了」;④剑气层 1~10 只喂 MP 条上的数字,世界里毫无表现。
- **素材来自作者自己**:m447 入库的 azure_soulblade bbmodel 里那五条骨骼动画,命名与职业技能一一对应
  (剑气波/招架反弹/剑气凌空/身法如风/万剑归一)。骨骼动画原版物品模型放不了(m447 已如实说明),
  但 m448 证明了**它的节拍能放**——这笔把剩下四条按同一条路子做完。
- **一处诚实的改编**:作者的动画都从「起手蓄势」开始(剑气波 0→0.38s、凌空 0→0.38s),而游戏里这些技能是
  **命中那一瞬间触发**的,起手其实是玩家自己那一刀、已经挥完了。所以统一只取「出刀之后」的段落,段内相对
  时长按原比例保留:剑气波取 0.55→1.4s 的 0.85s → 9t 推进(重音落在原 0.82 那一拍);招架反弹本就从接触
  瞬间起算、六拍 0/0.24/0.43/0.58/0.8/1.05 全取 → 21t;凌空取 0.38→1.65s 的 1.27s → 25t 四段。
- **四条编排**:①**剑气波**=冰蓝横扇 9t 推到 range,越远越宽、两端后掠下垂,第 5t 一记重音,末端拍散;
  ②**招架反弹**=面向攻击者立起刃壁 → 向内收束卸力 → 沿连线把力打回去(送回那一拍配 ATTACK_KNOCKBACK)→ 余劲 → 归位;
  ③**剑气凌空**=光刃沿视线逐段拉出(0→6t)→ 留痕向两侧鼓出(6→13t)→ 崩解成刃屑(13→20t)→ 消散(20→25t),
  **弹道在发射瞬间快照**(伤害本就沿这条线结算过,人走开光刃该留在原处);④**剑气层气场**=对位五条里唯一的
  loop 动画「身法如风」,24t 转一圈、环上光点数=当前层数。
- **顺手掐掉一个 bug 苗头**:气场原打算「满 10 层=上膛」,写完复查发现两处不成立——加层在同一次命中事件的
  **前半段**、凌空判定在**后半段**读 ≥10,所以层数刚变成 10 的**那一刀自己就洞穿了**,10 层从不停留一刀的时间;
  且 min(10,…) 封顶后每刀都会再得到 10,拿 10 当触发档等于**每刀重复放**。改成 **1/5/9 三档**,9=「下一刀
  蓄满即洞穿」才是正确播报点;也顺便避开了连击时 24t 任务层层叠加。上膛档用 END_ROD 白芒 + BELL_RESONATE
  pitch 1.8(与武僧金钟罩同音效 pitch 1.0 明显错开)。
- **不另开类,挂进 WeaponSkillFx(m255)**:守则第 4 条「不造新接口,先复用已有」——那边的多帧任务队列
  (含逐任务 try/catch)和 ring/n 画法助手正是要用的东西;而且**主类不必新增 register()**,少一处重复注册风险。
  两边各自独立开关互不影响。文件 235 → 424 行,类文档已注明新增托管范围。
- **纪律**:全服务端 spawnParticles(联机同屏可见,零客户端零协议);粒子/音效常量全部在树已用
  (SWEEP_ATTACK/SOUL_FIRE_FLAME/ENCHANTED_HIT/END_ROD/CRIT · ATTACK_SWEEP/STRONG/CRIT/KNOCKBACK/BELL_RESONATE),
  用到的 Vec3d.length()/crossProduct/lengthSquared/getEyePos/getStandingEyeHeight/getRotationVector/MathHelper.clamp/
  getYaw 逐个 grep 到在树先例,**零新 API 面、零待编译验证**。伤害/判定/耗层四处一律不碰,**纯演出、零平衡改动**;
  `swordsmanFancyFx false` 逐字回 m208/m234 的单帧简版。
- **顺手补一处健壮性**:直视天顶/脚底时 `look×(0,1,0)` 退化成零向量、`normalize()` 返回 ZERO 会把整个扇面压成
  一点。新增 rightOf() 用偏航角兜一个任意水平单位向量(此时任何水平方向都与视线正交)。m255 混沌斩有同一处
  潜在退化但从没人报过,这次只在新代码里兜,不动在树已验证的老路径。
- **本轮先把 m450a 那个真病根堵了**:m450a 查出「沙箱只装 JRE 无 javac,`javac:not found` 被 grep 过滤器吞掉,
  所有 javac 筛查静默假通过」。新会话沙箱**默认仍然没有 javac**——本轮开工先 `apt-get update` + 装
  openjdk-21-jdk-headless(21.0.11)才动手。三文件真跑全量编译:397 个错误**逐条确认全部是外部依赖噪音**
  (package does not exist / cannot find symbol),非依赖类错误 **0 条**;括号平衡三文件全 OK;两个资源校验脚本
  只剩文档已注明的 pack.mcmeta 假阳性。
- 配置+3 **configVersion 169→170**(swordsmanFancyFx / swordsmanFxScale / swordsmanEdgeAuraFx;均为新字段,
  老档缺字段自动取 Java 默认,无需迁移块)。
- 实机盯:①剑客持剑蓄满劈怪 → 身前该甩出一道**推得出去的**冰蓝横扇(不再是原地一小撮),中途一记重音;
  ②举盾挨近战 → 刃壁 + 一条打回攻击者的连线;③攒到 9 层听见高音铃看到白芒,下一刀蓄满放凌空 → 光刃拉出后
  **留一会儿再崩解**,人走开光刃留在原处;④`swordsmanFxScale 2` 看夸张版、`swordsmanFancyFx false` 逐帧回旧样。

## m452 MC百科更新日志(作者:「这一段时间更新了什么,可以写进 MC百科的」,2026-08-04)
- **范围判定**:仓库里上一版对外文案 `docs/MC百科介绍.md` 最后动在 m400,且正文内容 grep 验证连伤害飘字/掉落光柱/
  转场这些 m373 起的东西都没写——所以「这一段时间」取 **m373~m451**(7/30~8/4 的整波「观感与手感」更新)。
- **产出**:`docs/MC百科更新日志_2026年8月观感手感大更新.md`(可直接粘进百科「模组动态/更新日志」)+ 同内容
  .docx 交付作者(铁律 6:文档输出 Word)。玩家侧口径六大块:战斗打击感 7 条 / 职业体验 7 条 / 世界氛围 5 条 /
  界面 HUD 8 条 / 数值便利 6 条 / 重要修复 4 条,附「给老玩家的话」(v170 迁移只动旧默认、独立开关与倍率、fxtest 速览)。
- **口径纪律**:只收玩家可感知的改动,内部重构(m428)/打点(m427)/CI 修复(m412/m450a)/最终被还原的武器试点
  (m442~m449,净效果=原剑+低段爬色+幻影剑群,只写留下来的三样)一律不进正文;m426 下架+m439 重新上架净效果为零,
...
## m454 对外文档六职业口径同步(作者贴来 MC百科正文点名修改,2026-08-04)
- **作者的百科正文只需三处**:①标题 [h2=七大职业]→[h2=六大职业](作者其实已经删了表格里的召唤师行,只剩
  标题没跟上);②常见问题**新增「以前玩的召唤师怎么办」**——老玩家会实打实撞上迁移,FAQ 必须写明
  (免费重选本命/旧鹰扬自动转换零损失/暗影分身不受影响);③其余逐字未动。修订版全文入库
  `docs/MC百科正文_六职业版.md`([h1]/[h2] 标记原样,可整篇替换粘回百科)+ .docx 交付(铁律 6)。
- **仓库三份旧文案同步**:MC百科介绍.md(七大→六大/30字版七职业→六职业/删表行)、游玩攻略.md(删表行)、
  m452 更新日志(删干弟自定义条目与修复行、"七职业武器"改"全部职业武器",**追记「召唤师移除」公告段**——
  玩家照日志找功能,移除必须在日志里有出处)。游玩介绍.md 本来就是 6 职业老口径,恰好不用动;
  DEVLOG/HANDOVER 的历史记载保留不改。
- 纯文档零代码,配置 v 仍 171。
