...
- **装备无限强化 + 品质等级**:把**武器/盔甲**拿材料一直喂,强化等级**无上限**,加成绑在物品上。品质 **普通 → 精良 → 优秀 → 精锐 → 稀有 → 史诗 → 传说 → 神器 → 至尊** 共 9 阶,随强化等级晋升,越高阶解锁更高**暴击率 / 攻速**(武器暴击会追加伤害)。工作台放 1 件装备 + 材料(生命碎片 +1 / 结晶 +10 / 核心 +100 / 灾变血核 +1000,每格 1 个)→ 等级累加;武器加攻击/攻速/暴击/耐久,盔甲加护甲/韧性/最大生命/耐久。tooltip 显示品质与强化等级;**背包「装备」按钮**打开武器介绍界面(品质·基础属性·稀有度·下一阶进度)。`/yongye enhance <等级>` 可直接设定。
