 ...
 * 两个方向,覆盖三类己方召唤物(铁傀儡 / 肝帝天团 / 术士暗影分身,与免友伤 m299 同一套判定):
 ...
 * 实现要点:不动召唤物 AI goal 结构(零 mixin),直接 setTarget 交给原版攻击 goal 接手;铁傀儡这类
 * Angerable 额外 setAngryAt+setAngerTime(否则下 tick 会被它自己的仇恨 goal 抢回目标)——
 * Angerable 接口与两方法名已按 FabricMC/yarn 1.21.1 官方映射核对(class_5354),零编译风险。
...
public final class SummonAssistHandler {
    ...
    private static void retarget(ServerWorld sw, ServerPlayerEntity owner, LivingEntity target, boolean force) {
        ...
        for (MobEntity mob : sw.getEntitiesByClass(MobEntity.class, box, m -> m.isAlive() && isOwnSummon(m, id))) {
            ...
            if (mob instanceof Angerable ang) {               // 铁傀儡:压住自身仇恨 goal 防抢回目标
                ang.setAngryAt(target.getUuid());
                ang.setAngerTime(400);
            }
        }
    }
    ...
    private static boolean isOwnSummon(Entity e, UUID owner) {
        if (e instanceof com.yongye.entity.GanDiEntity g) return owner.equals(g.getOwner());
        if (e instanceof com.yongye.entity.WarlockCloneEntity w) return owner.equals(w.getOwner());
        if (e instanceof IronGolemEntity ig && ig.getCommandTags().contains(SummonerHandler.TAG)) {
            return owner.equals(SummonerHandler.ownerOf(ig));
        }
        return false;
    }
}
