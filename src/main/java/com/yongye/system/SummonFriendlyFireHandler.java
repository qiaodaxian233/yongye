 ...
 * 三类己方召唤物一并治:召唤师铁傀儡(tag yongye_summon)/ 肝帝天团 GanDiEntity / 术士暗影分身 WarlockCloneEntity
 * ——分身术核验结论:WarlockCloneEntity 是普通 PathAwareEntity,同样吃主人横扫,**同病一并修**。
 *
 ...
 * (联机队友的横扫同理),且 GanDi/分身也没有公开 owner 取值口——按全体玩家免最稳。
...
public final class SummonFriendlyFireHandler {
    ...
    public static void register() {
        ServerLivingEntityEvents.ALLOW_DAMAGE.register((entity, source, amount) -> {
            ...
            boolean ours = entity instanceof com.yongye.entity.GanDiEntity
                    || entity instanceof com.yongye.entity.WarlockCloneEntity
                    || (entity instanceof IronGolemEntity g && g.getCommandTags().contains(SummonerHandler.TAG));
            if (!ours) return true;
            return !(source.getAttacker() instanceof PlayerEntity); // 玩家致伤 = 误伤,取消;其余照常
        });
        Yongye.LOGGER.info("[夜蚀] 召唤物免友伤已挂载(傀儡/肝帝/暗影分身)");
    }
}
