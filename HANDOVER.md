...
- **项目完成度(估)≈ 85%**:核心玩法、成长线、世界节奏、Boss、HUD、饰品栏、时间进度、职业系统骨架均已落地;余下主要是职业的深化(加点/技能树/专属机制)、若干美术占位替换、以及少量需 mixin 的精细机制。
...
### 新增待办(最近几轮产生,优先级高→低)
- **职业系统深化(下一步重点)**:用户要「职业升级可加点 / 可学技能」——需做一套天赋点/技能树(看自由发挥)。当前 m37 只实现了 6 职业的属性修饰 + 等级门控(50/100,降级失去) + 武僧空手叠拳伤 + 刺客夜视。
- **职业专属机制未完**:武僧「武器耐久×2」(需 mixin 拦截耐久消耗)、刺客「专属暴击」(暂用攻击加成代替)、术士「远程伤害专属加成」(暂用攻击+交互距离代替)。
- **选职业 GUI**:用户提过「开局选职业送武器」备选方案 + 一个选职界面,未做。
- **职业系统编译点**:`ClassManager` 用了 `EntityAttributes.PLAYER_BLOCK_INTERACTION_RANGE / PLAYER_ENTITY_INTERACTION_RANGE`,1.21.1 若字段名不符需改;`p.experienceLevel` / `setExperienceLevel` 已用。
