...
- **项目完成度(估)≈ 87%**:核心玩法、成长线、世界节奏、Boss、HUD、饰品栏、时间进度、职业系统、天赋树(命令版第一版)均已落地;余下主要是职业专属机制(部分需 mixin)、选职/天赋 GUI、天赋数值平衡、若干美术占位替换。
...
- **属性字段编译点**:m37 两交互距离属性 `PLAYER_BLOCK/ENTITY_INTERACTION_RANGE` 已随 build 验证通过;m40 `TalentManager` 新用 `GENERIC_ATTACK_SPEED / GENERIC_KNOCKBACK_RESISTANCE / GENERIC_ARMOR_TOUGHNESS / GENERIC_LUCK`,1.21.1 若字段名不符则改;`p.experienceLevel` / `setExperienceLevel` 已用。
